...
class Core
{
    ...
    public function getAccountJsSymbols()
    {
        if (! class_exists(\NumberFormatter::class)) {
            return [
                'symbol'  => $this->fallbackCurrencySymbol($this->getCurrentCurrencyCode()),
                'decimal' => '.',
                'format'  => '%s%v',
            ];
        }
    ...
    public function formatPrice($price, $currencyCode = null)
    {
        ...
        if (! class_exists(\NumberFormatter::class)) {
            $symbol = $currency->symbol ?: $this->fallbackCurrencySymbol($currency->code);
            $formattedPrice = number_format((float) $this->convertPrice($price), 2, '.', ',');

            return trim($symbol . ' ' . $formattedPrice);
        }
    ...
    public function formatBasePrice($price, $isEncoded = false)
    {
        ...
        if (! class_exists(\NumberFormatter::class)) {
            ...
            $content = trim($this->fallbackCurrencySymbol($this->getBaseCurrencyCode()) . ' ' . $formattedPrice);
    ...
    /**
     * Fallback symbol map when intl is unavailable.
     */
    protected function fallbackCurrencySymbol(string $currencyCode): string
    {
